...
async fn main() {
    ...
    let parquet_object_writer = ParquetObjectWriter::new(store.clone(), Path::from("test1.parquet"));
    ...
    let parquet_object_writer = ParquetObjectWriter::new(store.clone(), Path::from("test2.parquet"));
    let mut parquet_writer = AsyncArrowWriter::try_new(
        parquet_object_writer,
        schema.clone(),
        Some(WriterProperties::default())
    ...
    ctx.register_parquet(table_ref, PARQUET_DIR, ParquetReadOptions::default()).await.unwrap();

    let batches = ctx.sql("select * from test7").await.unwrap().collect().await.unwrap();
